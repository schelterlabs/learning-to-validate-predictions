...
def generate_training_data():
    ...
    test_data = pd.read_csv('./datasets/adult/adult.csv')
    ...
    if not os.path.exists("./perturbed_datasets/"):
        os.mkdir('./perturbed_datasets')

    pd.concat(corrupted_test_data_list).\
        to_csv('./perturbed_datasets/adult_all_shifts_combined.csv')
...
def train_meta_regressor(predictions_perturbed, perturbation_ids_train, perturbation_ids_valid):
    ...
    for perturbation_id in sorted(predictions_perturbed.perturbation_id.unique()):
        ...
        predictions_proba = np.array(predictions[['class_>50K_score', 'class_<=50K_score']])
        features = compute_features(predictions_proba)
        num_features = features.shape[0]

        predictions['prediction'] = predictions['class_<=50K_score'].apply(lambda row: '<=50K' if row > .5 else '>50K')

        score_on_corrupted_test_data = scoring(predictions['class'], predictions['prediction'])
...
if __name__ == "__main__":
    ...
    # 3) Load predictions
    filenames_to_load = []
    for (dirpath, dirnames, filenames) in os.walk('./perturbed_datasets/all_shifts_combined_predictions'):
        for filename in filenames:
            if 'table' in filename:
                filenames_to_load.append(filename)

    predictions_perturbed = pd.concat(
        [pd.read_csv(os.path.join('./perturbed_datasets/all_shifts_combined_predictions', filename))
    ...
    ax.set_title('Cloud AutoML performance predictor on adult data')
    fig.savefig('./artifacts/cloud_service/validation.png')
    fig.show()
