...
class MissingValuesInSlice:

    def __init__(self, row_predicate, columns, value_to_put_in, info):
    ...
    def transform(self, clean_df):
        ...
        df.loc[self.predicate(df), self.columns] = self.value_to_put_in
...
#
#     def __init__(self, row_predicate, columns):
#         self.predicate = row_predicate
#         self.columns = columns
#
#     def transform(self, clean_df):
...
def add_perturbations(_perturbations, _predicate, info):
    for _num_columns_affected in range(1, len(categorical_columns) + 1):
        for _ in range(0, num_repetitions):
            ...
            _perturbations.append(MissingValuesInSlice(_predicate, _columns_affected, 'NULL', info))

            # _perturbations.append(Mixture([
            #     MissingValuesInSlice(_predicate, _columns_affected, 'NULL')#,
            #     #OutliersInSlice(_predicate, _numerical_columns_affected)
            # ]))
...
for model_to_evaluate in models_to_evaluate:
    ...
    k = 3
    marital_statuses = test_data['marital_status'].value_counts()[:k].index.tolist()
    educations = test_data['education'].value_counts()[:k].index.tolist()
    ...
    for marital_status in marital_statuses:
        ...
        add_perturbations(perturbations, predicate, "marital_status=="+marital_status)

    for education in educations:
        predicate = lambda df: (df.education == education)
        add_perturbations(perturbations, predicate, "education=="+education)

    for marital_status in marital_statuses:
        for education in educations:
            predicate = lambda df: (df.marital_status == marital_status) & (df.education == education)
            add_perturbations(perturbations, predicate, "marital_status=="+marital_status+"&eduction=="+education)
